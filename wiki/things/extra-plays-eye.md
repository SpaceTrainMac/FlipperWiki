---
title: The EXTRA PLAYS eye
type: thing
aliases: [EXTRA PLAYS, the winking eye, the eye]
body: game
---
# The EXTRA PLAYS eye

**The digit under `EXTRA PLAYS` shows a `0`, and on seven segments a `0` is an open eye — which
winks** ([0093](../../raw/rules/0093-the-displays.md), [0097](../../raw/rules/0097-extra-plays-is-a-joke-and-deliberately.md)):

| | Shows | For |
|---|---|---|
| at rest | `0` | 20 seconds |
| a wink | `-` | 300 ms |
| **every tenth wink**, instead | `-`, `0`, `-` | 300 ms, 200 ms, 300 ms |

*(the table is rule 0097's, rendered for a player)*

**So it blinks once every twenty seconds and twice about every three and a half minutes** — often
enough to catch out of the corner of your eye, rare enough never to compete with the playfield
([0097](../../raw/rules/0097-extra-plays-is-a-joke-and-deliberately.md)).

**It is a joke, and deliberately.** The machine is played at home and free, so there are no plays to
owe, and a permanently dark digit would say less than a cheerful one ([0097](../../raw/rules/0097-extra-plays-is-a-joke-and-deliberately.md)). **It is the one thing
on this machine that is not a rule**: nothing lights it, nothing reads it, and no score depends on
it ([0097](../../raw/rules/0097-extra-plays-is-a-joke-and-deliberately.md)).
